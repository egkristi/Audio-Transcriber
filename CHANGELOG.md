...
- **Phase 11: Fully automated pipeline** — language and dialect auto-detection with model routing. (ROADMAP.md Phase 11)
  - **`src/model_registry.py`** — new module mapping language codes ("no", "nn", "sv", "da", "en") to optimal transcription and alignment models. Provides `get_model_config()`, `get_available_languages()`, `get_alignment_model()`, and `resolve_language()` with confidence-based routing (≥0.8 high, ≥0.5 medium, <0.5 fallback to Norwegian).
