...
6. **Language detection is unreliable with tiny model.** `detect_language()` returns "et" (Estonian) for Norwegian with confidence 0.29. Always use `language="no"` for Norwegian production workloads until fixed.
7. **Loudness normalization can clip.** Target -16 LUFS is too aggressive for high-dynamic-range recordings. Peak 0.66 → 1.91 after gain. Use -20 LUFS or limit gain to 1.0.
8. **~400 of 410 test files are corrupted** ("moov atom not found"). This is a data quality issue, not a code bug. Filter files < 1 KB before batch processing.
